    ...
    def write_header(
        ...
        header: Dict[str, Any] = {
            ...
            "config": {
                ...
                "energy_capacity": env.config.energy_capacity,
                "training_energy_footprint": env.config.training_energy_footprint,
                "upskill_energy_footprint": env.config.upskill_energy_footprint,
                "intensive_work_energy_footprint": env.config.intensive_work_energy_footprint,
...
def build_agent(args, config=None) -> object:
    if args.agent == "llm":
        ...
        prompt = _build_system_prompt(
            ...
            energy_threshold=config.energy_threshold_for_upskill if config else 0.30,
            upskill_energy_footprint=config.upskill_energy_footprint if config else 0.15,
            intensive_work_energy_footprint=config.intensive_work_energy_footprint if config else 0.20,
            training_energy_footprint=config.training_energy_footprint if config else 0.15,
        ...
        return LLMAgent(
            ...
            switch_base_cost=config.switch_occupation_base_cost if config else 2_000.0,
            energy_threshold_for_upskill=config.energy_threshold_for_upskill if config else 0.30,
            upskill_energy_footprint=config.upskill_energy_footprint if config else 0.15,
            intensive_work_energy_footprint=config.intensive_work_energy_footprint if config else 0.20,
            training_energy_footprint=config.training_energy_footprint if config else 0.15,
            occupations=occupation_details,
